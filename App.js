...
/// Example for Measure
import React, {Component, useRef, useState, createRef, useImperativeHandle} from "react";
import {
    View,
    Text,
    StyleSheet,
...
/*
/// Example for Composing Animations
import React, {useEffect, useRef, useState} from "react";
import {View, Text, Animated, StyleSheet} from 'react-native';

const Anima1 = (props) => {
    const pos = useRef(new Animated.ValueXY({x: 10, y: 10})).current;
    ...
    return (
        <Animated.View stype={[...props.style, {top: pos.x, left: pos.y}]}>
            {props.children}
        </Animated.View>
    );
};
    ...
    return (
        <View style={[styles.container]}>
            <ImageBackground
                ...
                <Text style={{alignSelf:'center', fontSize:50, color:'blue', fontWeight:'bold'}}>Inside</Text>
            </ImageBackground>
...
const BallComponent = props => {
    const finalPosition = useRef(new Animated.Value(0)).current;

    useEffect(() => {
        Animated.timing(finalPosition, {
            toValue: 300,
            duration: 5000,
            useNativeDriver: true,
        }).start();
    ...
    return (
        <Animated.View style={[...props.style, {top: finalPosition, backgroundColor: 'coral'}]}>
            {props.children}
        </Animated.View>
    );
};
...
export default function App() {
    return (
        <View style={[styles.container]}>
            <BallComponent style={[styles.ball]}>
...
const FadeAnimation = props => {
    ...
    return (
        <Animated.View style={[...props.style, {opacity: fadeVal}]}>
            {props.children}
        </Animated.View>
    )
};
export default function App() {
    ...
    return (
        <View style={[styles.page]}>
            <View style={[styles.container]}>
                <FadeAnimation style={[styles.button]}>
                    <TouchableOpacity>
